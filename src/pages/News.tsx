...
import { Clock, ArrowUpRight, ArrowLeft, BookOpen, Loader2 } from 'lucide-react';
          ...
          <AnimatePresence mode="wait">
            {selectedArticle ? (
              ...
              <motion.div key="list" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
                ...
                {articles.length === 0 ? (
                  <p className="text-center text-muted-foreground py-12">No hay noticias disponibles en este momento.</p>
                ) : (
                  <>
                    {/* Featured article */}
                    <motion.article
                    ...
                    >
                      <div className="grid grid-cols-1 md:grid-cols-2">
                        <div className="aspect-video md:aspect-auto overflow-hidden">
                          <img src={articles[0].image_url} alt={articles[0].title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                        </div>
                        <div className="p-6 md:p-8 flex flex-col justify-center">
                          <div className="flex items-center gap-2 mb-3">
                            <span className="text-xs font-medium px-2 py-1 rounded-lg bg-accent text-accent-foreground">{articles[0].category}</span>
                            <span className="text-xs text-muted-foreground flex items-center gap-1"><Clock className="w-3 h-3" />{articles[0].read_time}</span>
                          </div>
                          <h2 className="font-display font-bold text-xl md:text-2xl mb-3 group-hover:text-primary transition-colors">{articles[0].title}</h2>
                          <p className="text-sm text-muted-foreground mb-4">{articles[0].summary}</p>
