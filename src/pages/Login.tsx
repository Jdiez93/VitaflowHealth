...
const Login = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      toast({ title: 'Demo', description: 'Conecta Supabase para habilitar la autenticación.' });
      setLoading(false);
    }, 1000);
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-background relative overflow-hidden px-4">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md glass-card p-8">
        ...
        <h1 className="font-display font-bold text-2xl text-center mb-2">Bienvenido de vuelta</h1>
        <p className="text-sm text-muted-foreground text-center mb-8">Inicia sesión para continuar tu camino de bienestar</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <div className="relative mt-1">
              ...
              <Input id="password" type="password" placeholder="••••••••" value={password} onChange={e => setPassword(e.target.value)} className="pl-10 rounded-xl" required />
            </div>
          </div>
