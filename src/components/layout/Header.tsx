...
const Header = ({ isAuthenticated = false, userName = 'Usuario' }: HeaderProps) => {
      ...
      <div className="container mx-auto px-4 h-16 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-2">
          {isAuthenticated ? (
              ...
              <AnimatePresence>
                {dropdownOpen && (
                  <motion.div
                    ...
                    exit={{ opacity: 0, y: 8 }}
                    className="absolute right-0 top-full mt-2 w-48 glass-card p-2 shadow-lg"
                  >
                    ...
                    <button className="flex items-center gap-2 px-3 py-2 rounded-xl text-sm hover:bg-muted transition-colors w-full text-left text-destructive">
            ...
            <>
              <Link to="/login">
                <Button variant="ghost" size="sm" className="rounded-xl">Iniciar Sesión</Button>
              </Link>
              <Link to="/register">
                <Button size="sm" className="rounded-xl bio-gradient border-0 text-primary-foreground hover:opacity-90">Registrarse</Button>
              </Link>
            </>
          )}
        </div>
        ...
        {mobileOpen && (
          ...
          >
            <div className="container mx-auto px-4 py-4 flex flex-col gap-2">
              {publicLinks.map((link) => (
                <Link key={link.to} to={link.to} className="px-4 py-3 rounded-xl text-sm font-medium hover:bg-muted transition-colors" onClick={() => setMobileOpen(false)}>
                  {link.label}
                </Link>
              ))}
              {!isAuthenticated && (
                <div className="flex gap-2 pt-2 border-t border-border">
                  <Link to="/login" className="flex-1" onClick={() => setMobileOpen(false)}>
                    <Button variant="ghost" className="w-full rounded-xl">Iniciar Sesión</Button>
                  </Link>
                  <Link to="/register" className="flex-1" onClick={() => setMobileOpen(false)}>
                    <Button className="w-full rounded-xl bio-gradient border-0 text-primary-foreground">Registrarse</Button>
                  </Link>
                </div>
              )}
              {isAuthenticated && (
